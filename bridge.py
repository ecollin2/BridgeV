...
def main():
    ...
    avax_nonce = avax.eth.get_transaction_count(admin_address)
    bsc_nonce = bsc.eth.get_transaction_count(admin_address)

    for _, row in tokens_df.iterrows():
        token = Web3.to_checksum_address(row["address"])
        chain = row["chain"].strip().lower()

        if chain == "avax":
            try:
                print(f"Registering on source (avax): {token}")
                ...
                print(f"⚠️  Skipped or failed to register on source: {e}")

        if chain == "bsc":
            try:
                print(f"Creating wrapped token on destination (bsc): {token}")
                ...
                print(f"✅ Wrapped: {bsc.to_hex(tx_hash)}")
                bsc_nonce += 1
            except Exception as e:
                print(f"⚠️  Skipped or failed to create wrapped token: {e}")
